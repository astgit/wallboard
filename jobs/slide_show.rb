...
@SS = SlideShow.new()
# SCHEDULER.cron '*/30 * * * *' do |job|
#   settings = @SS.get_settings
#   settings.each do |widget, project|
#     puts DateTime.now.to_s+" Resizing images for #{widget}, #{project.to_s}"
#     @SS.resize_images(
#       @SS.get_dir_file_list(Dir.pwd+'/'+project['directory'], project['pattern'], project['subDirectoryExceptions']).shuffle,
#       widget,
#       Dir.pwd+'/'+project['directory'],
...
SCHEDULER.every '30s', :first_in => 0 do |job|
  settings = @SS.get_settings
  settings.each do |widget, project|
    @files = { widget => @SS.get_file_list(widget, project) }
